...
# ⭐ NEW: List of fully trained annotators to include in overall TPM calculation
TRAINED_ANNOTATORS = [
    "annotator42_lambda@encord.ai",
    "annotator52_lambda@encord.ai",
    "annotator61_lambda@encord.ai",
    "annotator110_lambda@encord.ai",
]

# Set to empty list to include ALL annotators (default behavior)
# TRAINED_ANNOTATORS = []
...
# Overall metrics (ALL annotators)
total_time_spent_seconds = 0.0
total_audio_duration_minutes = 0.0
matched_tasks = 0

# ⭐ NEW: Overall metrics (TRAINED annotators only)
trained_time_spent_seconds = 0.0
trained_audio_duration_minutes = 0.0
trained_matched_tasks = 0
...
for (project_hash, title), duration_seconds in task_durations.items():
    ...
    if key in project_task_time:
        ...
        if primary_user:
            ...
            # ⭐ NEW: Calculate trained annotator metrics
            # If TRAINED_ANNOTATORS is empty, include everyone
            # Otherwise, only include specified annotators
            if not TRAINED_ANNOTATORS or primary_user in TRAINED_ANNOTATORS:
                trained_time_spent_seconds += time_spent
                trained_audio_duration_minutes += audio_minutes
                trained_matched_tasks += 1
...
if total_audio_duration_minutes > 0:
    ...
    print(f"\n📊 Overall Average TPM (ALL annotators): {overall_tpm:.2f} minutes per audio minute")
...
# ⭐ NEW: Display trained annotator metrics
if TRAINED_ANNOTATORS:
    print(f"\n{'─' * 80}")
    print(f"🎯 TRAINED ANNOTATORS ONLY ({len(TRAINED_ANNOTATORS)} annotators)")
    print(f"{'─' * 80}")

    if trained_audio_duration_minutes > 0:
        trained_tpm = (trained_time_spent_seconds / 60.0) / trained_audio_duration_minutes
        print(f"\n✅ Average TPM (Trained only): {trained_tpm:.2f} minutes per audio minute")
        print(f"   Tasks matched: {trained_matched_tasks:,}")
        print(f"   Total work time: {trained_time_spent_seconds / 3600:.2f} hours")
        print(f"   Total audio: {trained_audio_duration_minutes / 60:.2f} hours")

        # Show difference
        if total_audio_duration_minutes > 0:
            diff = trained_tpm - overall_tpm
            diff_pct = (diff / overall_tpm) * 100
            print(f"\n   📈 Difference from overall: {diff:+.2f} min/min ({diff_pct:+.1f}%)")
    else:
        print("\n❌ No tasks found for trained annotators")

    print(f"\nTrained annotators list:")
    for email in TRAINED_ANNOTATORS:
        print(f"  • {email}")
...
for project_id in sorted(by_project.keys()):
    ...
    for annotator_key in project_annotators:
        ...
        if stats['audio_duration'] > 0:
            tpm = (stats['time_spent'] / 60.0) / stats['audio_duration']
            user_email = annotator_key[1]

            # ⭐ NEW: Mark trained annotators
            is_trained = "🎯" if (not TRAINED_ANNOTATORS or user_email in TRAINED_ANNOTATORS) else "⚠️"

            annotator_stats.append((user_email, tpm, stats, is_trained))

    annotator_stats.sort(key=lambda x: x[1], reverse=True)

    for user_email, tpm, stats, marker in annotator_stats:
        print(f"\n  {marker} Annotator: {user_email}")
        print(f"    TPM: {tpm:.2f} minutes/audio minute")
        print(f"    Tasks: {stats['task_count']}")
        print(f"    Work time: {stats['time_spent'] / 3600:.2f} hours")
        print(f"    Audio: {stats['audio_duration'] / 60:.2f} hours")
